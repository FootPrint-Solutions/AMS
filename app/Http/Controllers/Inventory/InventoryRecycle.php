...
class InventoryRecycle extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        // show all distributor shops in inventory details when qty sum is more than 0 and show the qty sum in stock column
        $data = InventoryRecycleDetailModel::with('distributorShop')
            ->select('distributor_shop_id', DB::raw('SUM(quantity) as total_quantity'))
            ->groupBy('distributor_shop_id')
            ->get();
        $rows = [];
        $no = $start + 1;
        foreach ($data as $key) {
            $row = [];
            $row[] = $no++;
            $row[] = $key->distributorShop ? $key->distributorShop->id : '-';
            $row[] = $key->distributorShop ? $key->distributorShop->name : '-';
            $row[] = $key->total_quantity;
            $rows[] = $row;
        }
    ...
    public function syncStock(Request $request)
    {
        ...
        // get All inventory details with distributor shop null and update the distributor shop with the latest distributor shop from sales order or purchase order
        $inventoryDetailsWithoutShop = InventoryRecycleDetailModel::whereNull('distributor_shop_id')->get();
        foreach ($inventoryDetailsWithoutShop as $detail) {
            if ($detail->salesOrderBattery) {
                $latestShopId = $detail->salesOrderBattery->salesOrder->distributor_shop_id ?? null;
                if ($latestShopId) {
                    $detail->distributor_shop_id = $latestShopId;
                    $detail->save();
                }
            } elseif ($detail->purchaseOrder) {
                $latestShopId = $detail->purchaseOrder->ship_to_id ?? null;
                if ($latestShopId) {
                    $detail->distributor_shop_id = $latestShopId;
                    $detail->save();
                }
