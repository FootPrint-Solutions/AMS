...
@section('content')
        ...
        <div class="card-body">
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title mb-0">Accounting Expense List</h3>
                    </div>
                    <div class="col-auto text-end float-end ms-auto download-grp">
                        <button id="btn-add" class="btn btn-primary btn-sm">
                            <i class="fas fa-plus"></i> Add New Accounting Expense
                        </button>
                    </div>
                </div>
            </div>
    ...
    <div class="card">
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped" id="table-accounting-expense">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>No</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            table = $('#table-accounting-expense').DataTable({
                lengthMenu: [
                    [5, 10, 25],
                    [5, 10, 25]
                ],
                ...
                buttons: [{
                        ...
                        action: function() {
                            ...
                            if (selectedRows.length !== 1) {
                                Swal.fire('Error', 'Please select one accounting expense to edit.',
                                    'error');
                                return;
                            }

                            const id = selectedRows[0][0];
                            goToPage('/accounting-expense/edit/' + id);
                        }
                    },
                    {
                        ...
                        action: function() {
                            ...
                            if (selectedRows.length === 0) {
                                Swal.fire('Error', 'Please select at least one accounting expense.',
                                    'error');
                                return;
                            }
                            ...
                            Swal.fire({
                                title: 'Delete Accounting Expense',
                                text: 'Are you sure you want to delete selected accounting expense(s)?',
                                icon: 'warning',
                                ...
                                $.ajax({
                                    url: '/accounting-expense/destroy',
                                    type: 'POST',
                                    data: {
                                        _token: '{{ csrf_token() }}',
                                        ids: ids
                                    ...
                                    error: function() {
                                        Swal.fire('Error',
                                            'Failed to delete accounting expense.',
                                            'error');
                                    }
                                });
                            });
                    ...
                    },
                    {
                        ...
                        action: function() {
                            ...
                            Swal.fire({
                                title: 'Post Accounting Expense',
                                text: 'Are you sure you want to post selected accounting expense(s)?',
                                icon: 'question',
                                showCancelButton: true,
                                ...
                                $.ajax({
                                    ...
                                    error: function() {
                                        Swal.fire('Error',
                                            'Failed to post accounting expense.',
                                            'error');
                                    }
                                });
                            });
                ...
                language: getDatatablesLanguangeConfigurations('Accounting Expense'),
                select: true,
            });

            $('#table-accounting-expense tbody').on('click', 'td.dt-control', function() {
                ...
                if (row.child.isShown()) {
                    ...
                    var accountingExpenseId = table.row(tr).data()[0];

                    $.ajax({
                        url: '/accounting-expense/items/' + accountingExpenseId,
                        method: 'GET',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
            ...
            $('#btn-add').on('click', function() {
                goToPage('/accounting-expense/create');
            });
